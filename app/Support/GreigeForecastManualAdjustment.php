...
/**
 * 生機月末在庫予想の手動調整（デモ用 JSON）。
 */
class GreigeForecastManualAdjustment
{
    private const FILE = 'greige_forecast_manual_adjustments.json';
    ...
    public static function totalFor(string $greigeSku, string $targetYm): float
    {
        return (float) collect(self::all())
            ->filter(fn ($a) => (string) ($a['greige_sku'] ?? '') === $greigeSku)
    ...
    public static function add(string $greigeSku, string $targetYm, float $qtyM, string $direction, string $reason, string $createdBy): object
    {
