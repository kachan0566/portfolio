...
染機投入予定日（生機月末予想の出荷側）は **製品発注の `contact_date`** を染機投入予定として使用（暫定）。専用列 `dye_input_date` は将来検討。
...
| 発注一覧 | 1発注1行 | **済** 明細行ごとに1行（`PurchaseOrder::displayLineList()`） |
...
| 段階 | 内容 | 依存 | デフォルト位置 | 状態 |
| --- | --- | --- | --- | --- |
| 8a | 入荷・発注の複数明細行 UI（同一種別のみ） | 6 | 本線8の直後 | **済 2026-07** |
